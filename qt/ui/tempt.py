...
class UiTempTxt(UiFrame):
    ...
    def draw(self, ui, d):
        ...
        if daily:
            ...
            for i in range(cnt):
                x1 = ui.canvas.dim.x * i // cnt + half
                y1 = self.dim.y - _CHART_OVERSHOT
                
                weather = forecast[i]
                temp    = weather.temp
                
                ui.text_center(50, '{:.0f} °C'.format(temp['max']), Vect(x1, y1 - 90))
                ui.text_center(50, '{:.0f} °C'.format(temp['min']), Vect(x1, y1 - 42))
        else:
            temp_max = -273.0
            temp_min =  273.0
