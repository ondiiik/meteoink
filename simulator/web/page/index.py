...
def index(page):
    ...
    with page.table(('SSID', 'BSSID', trn('Location'), '', ''), 'frame="hsides"', 'style="text-align:left"') as table:
        for i in connection.CONNECTIONS:
            ...
            table.row((i.ssid,
                       bssid,
                       location.LOCATIONS[loc].name if loc < len(location.LOCATIONS) else '...',
    ...
    with page.table(None, 'frame="hsides"') as table:
        table.row((trn('Outside temperature balanced'), web.button_enable(beep.TEMP_BALANCED, 'alerttemp')), web.SPACES)
        table.row((trn('Software bug detected'),        web.button_enable(beep.ERROR_BEEP,    'alertbug')), web.SPACES)
    ...
    with page.table(None, 'frame="hsides"') as table:
        ...
        table.row((trn('Password'), spot.PASSWD, web.button(trn('Edit'), 'passedt')), web.SPACES)
