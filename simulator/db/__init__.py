...
need_reset |= init(
    "sys", lambda: {"DHT_HUMI_CALIB": (1, 0), "EXCEPTION_DUMP": 0, "VERBOSE_LOG": False}
)
...
need_reset |= init(
    "spot",
    lambda: {
        "SSID": "metoink_" + "".join([choice(_chars) for _ in range(4)]),
        "PASSWD": "".join([choice(_chars) for _ in range(12)]),
    },
)
need_reset |= init(
    "temp", lambda: {"INDOOR_HIGH": 26, "OUTDOOR_HIGH": 27, "OUTDOOR_LOW": -5}
)
need_reset |= init("time", lambda: {"WINTER": False})
need_reset |= init("vbat", lambda: {"LOW_VOLTAGE": 3.2, "SHOW_VOLTAGE": False})
need_reset |= init(
    "api", lambda: {"APIKEY": "", "UNITS": "metric", "LANGUAGE": "EN", "VARIANT": 4}
)
