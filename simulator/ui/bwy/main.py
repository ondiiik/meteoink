...
class Epd(EpdBase):
    ...
    def repaint_forecast(self, volt):
        with self.Drawing("weather", self):
            if self.connection is None:
                ...
                out_temp = UiOutTemp(
                    self, V(weather.right + 5, 0), V(160, weather.height // 2)
                )
                in_temp = UiInTemp(
                    self, V(weather.right + 5, out_temp.bellow), out_temp.dim
                )
                outside = UiOutside(
                    self,
                    V(out_temp.right + 5, 0),
                    V(self.width - out_temp.right - 6, out_temp.height),
                )
                inside = UiInside(
                    self, V(in_temp.right + 5, outside.bellow), outside.dim
                )
                calendar_head = UiCalendar(
                    self, V(0, weather.bellow), V(self.width, 32)
                )
                icons = UiIcons(self, V(0, calendar_head.bellow + 6), V(self.width, 55))
                calendar_tail = UiCalendar(
                    self,
                    V(0, icons.bellow),
                    V(self.width, self.height - icons.bellow - 30),
                )
                ...
                wind = UiWind(
                    self,
                    V(0, calendar_tail.bellow),
                    V(self.width, self.height - calendar_tail.bellow),
                )
    ...
    def repaint_config(self, volt):
        ...
        with self.Drawing("hotspot", self):
            ...
            UiVBat(
                self, V(self.canvas.width // 2 - 10, self.canvas.height // 2), V(20, 10)
            ).repaint(volt)
