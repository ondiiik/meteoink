    ...
    def repaint_forecast(self, volt):
        with self.Drawing('weather', self):
            if self.connection is None:
                ...
                calendar_tail = UiCalendar(self, V(0, icons.bellow + 12), V(self.width, self.height - icons.bellow - 90))
                clouds = UiClouds(self, V(0, calendar_tail.above + 8), V(self.width, 30))
                graph_temp = UiTempGr(self, V(0, clouds.bellow + 12), V(self.width, calendar_tail.height - clouds.height))
